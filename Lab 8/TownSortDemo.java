...
public class TownSortDemo {
    public static void main(String[] args) {
        ...
        System.out.println("\nSorted ArrayList");
        displayArrayList(towns1);
        String searchName = JOptionPane.showInputDialog("Please enter a search name");
        Town searchTown = new Town(searchName, null, 0);

        int result = Arrays.binarySearch(towns, searchTown);
        if(result < 0){
            JOptionPane.showMessageDialog(null, "Town not found");
        }

        else {
            JOptionPane.showMessageDialog(null, "County: " +towns1.get(result).getCounty() + ", Name:  " + towns1.get(result).getName() + ", Population: " + towns1.get(result).getPopulation()); //Expect 4
        }
    }
